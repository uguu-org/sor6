...
if [[ $# -ne 2 && $# -ne 6 && $# -ne 7 ]]; then
   echo "$0 {input.svg} {output.png} [x1] [y1] [x2] [y2] [dpi]"
   exit 1
fi
INPUT=$1
OUTPUT=$2
...
# Directory where lock file and cache files will be placed.
#
# Note that this is derived from output file name.  The expectation is
# that all output will be written to the same directory.  Locking
# woudln't work if multiple output directories are used.
...
PREFIX="$OUTPUT_DIR/t_svg_cache"
CACHED_OUTPUT="${PREFIX}_${INPUT_HASH}.png"
EXPORT_DPI="--export-dpi=96"
if [[ $# -eq 6 ]]; then
   X1=$3
   Y1=$4
   X2=$5
   ...
   if [[ $# -eq 7 ]]; then
      ...
      X2=$5
      Y2=$6
      EXPORT_DPI="--export-dpi=$7"
      AREA="--export-area=$X1:$Y1:$X2:$Y2"
      FULL_PAGE_CACHE="$CACHED_OUTPUT"
      CACHED_OUTPUT="${PREFIX}_${INPUT_HASH}_${X1}_${Y1}_${X2}_${Y2}_${7}.png"
   else
...
flock "$OUTPUT_DIR/t_inkscape.lock" "$INKSCAPE" \
   "$AREA" \
   "$EXPORT_DPI" \
   --export-type=png \
